...
<template>
  <v-card class="mx-auto mb-3 card_post" max-width="600">
    ...
    <v-card-text color="deep-purple-lighten-2" variant="text">
      <div class="font-weight-bold text-h7">{{ item.user_id.email }}</div>
      <div class="font-weight-thin">{{ timeStamp }}</div>
    </v-card-text>

    <v-card-actions style="padding: 10px 12px">
      <v-btn color="orange"> {{ item.category_id.name }}</v-btn>

      <v-btn color="orange"> Khám Phá </v-btn>
    </v-card-actions>
  </v-card>
</template>
